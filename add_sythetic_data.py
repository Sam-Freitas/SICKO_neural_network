...
output_img_blank = r'C:\Users\LabPC2\Desktop\_SICKO_NN\training\images_synthetic_blank'
output_mask_blank = r'C:\Users\LabPC2\Desktop\_SICKO_NN\training\masks_synthetic_blank'
output_worms = r'C:\Users\LabPC2\Desktop\_SICKO_NN\training\isolated_worms'
output_img2 = r'C:\Users\LabPC2\Desktop\_SICKO_NN\training\images_synthetic'
output_mask2 = r'C:\Users\LabPC2\Desktop\_SICKO_NN\training\masks_synthetic'
...
all_imgs = natsorted(glob.glob(os.path.join(r'C:\Users\LabPC2\Desktop',r'_SICKO_NN\training\images','*.png')))#[:100]
...
for i,(this_img_path,this_mask_path) in enumerate(tqdm.tqdm(zip(all_imgs,all_masks), total=len(all_imgs))):
    ...
    if '_' not in img_name:
        ...
        noise_whole = random_noise(blank_mask, mode = 'gaussian', var = estimated_sigma)
        noise_mask = noise_whole*large_mask

        dst = cv2.inpaint(img,large_mask,9,cv2.INPAINT_TELEA).astype(np.float64) # cv2.INPAINT_NS #
        dst[large_mask>0] = ((noise_whole[large_mask>0]*128) + dst[large_mask>0])/2 # masking the noise to the same frequescy
...
all_isolated_worms = natsorted(glob.glob(os.path.join(r'C:\Users\LabPC2\Desktop',r'_SICKO_NN\training\isolated_worms','*.png')))
all_blank_imgs = natsorted(glob.glob(os.path.join(r'C:\Users\LabPC2\Desktop',r'_SICKO_NN\training\images_synthetic_blank','*.png')))
